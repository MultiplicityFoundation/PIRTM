...
def _info(residual: float = 0.1) -> StepInfo:
    return StepInfo(
        step=0, q=0.4, epsilon=0.05, nXi=0.2, nLam=0.2, projected=False, residual=residual
    )
...
def test_beneficence_check_fails_on_growth_or_residual():
    ...
    ok, detail = beneficence_check(
        X_t, X_next, _info(residual=11.0), norm_growth_limit=1.1, residual_limit=10.0
    )
    assert not ok
